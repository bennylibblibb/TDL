namespace CENTASMS.BLL
{
    ...
    public class SmsMsgReceHists
    {
        ...
        public SmsMsgReceHists()
        {
            ...
            this.dsCommand.TableMappings.Add("Table", "SMS_MSGRCV_HIST");
        }
        ...
        public SmsMsgReceHistData GetAllSmsReceHistData(string strCondition)
        {
            if (this.dsCommand == null)
            {
                throw new ObjectDisposedException(base.GetType().FullName);
            }
            SmsMsgReceHistData dataSet = new SmsMsgReceHistData();
            this.dsCommand.SelectCommand = this.GetLoadCommand();
            this.dsCommand.SelectCommand.CommandText = "SELECT * from SMS_MSGRCV_HIST   " + strCondition + " order by IREC_NO asc";
        ...
        private FbCommand GetInsertCommand()
        {
            ...
            {
                string cmdText = " INSERT INTO  SMS_MSGRCV_HIST  ( IREC_NO ,  CBATCHID ,  CMOBILENO,CGROUP_NAME ,  IPRIORITY   ) VALUES (?,?,?,?,?) ";
        ...
        public int GetNewIrec_No()
        {
            ...
            SmsMsgReceHistData dataSet = new SmsMsgReceHistData();
            this.dsCommand.SelectCommand = this.GetLoadCommand();
            this.dsCommand.SelectCommand.CommandText = "select  max(IREC_NO) IREC_NO from  SMS_MSGRCV_HIST  order by IREC_NO desc";
            this.dsCommand.Fill(dataSet);
            if (dataSet.Tables["SMS_MSGRCV_HIST"].Rows[0][0] == DBNull.Value)
            {
                return 1;
            }
            return (Convert.ToInt32(dataSet.Tables["SMS_MSGRCV_HIST"].Rows[0][0]) + 1);
        }

        public bool InsertSmsMsgReceHist(SmsMsgReceHistData smsmsg)
        {
            try
            {
                if (this.dsCommand == null)
                ...
                this.dsCommand.Update(smsmsg, "SMS_MSGRCV_HIST");
            }
            catch (Exception exception)
            {
                string str = exception.ToString();
                Files.CicsWriteError(DateTime.Now.ToString("HH:mm:ss") + "   SmsMsgReceHistData  " + exception.ToString());
            }
            if (smsmsg.HasErrors)
            {
                smsmsg.Tables["SMS_MSGRCV_HIST"].GetErrors()[0].ClearErrors();
                return false;
            }
        ...
        public SmsMsgReceHistData LoadMsgReceDataByIREC_NO(string irec_no)
        {
            if (this.dsCommand == null)
            {
                throw new ObjectDisposedException(base.GetType().FullName);
            ...
            SmsMsgReceHistData dataSet = new SmsMsgReceHistData();
            this.dsCommand.SelectCommand = this.GetLoadCommand();
            this.dsCommand.SelectCommand.CommandText = "SELECT * from SMS_MSGRCV_HIST    WHERE IREC_NO= '" + irec_no + "'  ";
            this.dsCommand.Fill(dataSet);
            return dataSet;
        }
    }
