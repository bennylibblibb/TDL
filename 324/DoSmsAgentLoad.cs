                ...
                try
                {
                    ...
                    DataTable table = connection.GetOleDbSchemaTable(OleDbSchemaGuid.Tables, null);
                     ...
                     int iSign = 0;
                         int ilen = 0;
                            int maxlen = 0;
                            string msg =this.doSmsContent ;//dataSet.Tables[0].Rows[0][2].ToString().Trim().Replace("SMS:", "");
                     ...
                     foreach (DataRow name in table.Rows)
                    {
                        try
                        {
                            ...
                            {
                                if ((dr.ItemArray.Length < 4 || dr[0].ToString() == "" || dr[1].ToString() == "" || dr[2].ToString() == "" || dr[3].ToString() == "") && bResult==false)
                                {
                                    error = true; 
                                   throw (new Exception ( "格式文件錯誤!"));
                                }
                                string strMobile = dr[0].ToString().Trim();
                                ...
                                if (strMobile != "")
                                {
                                 ...
                                 {
                                  string strMsg=msg.Replace("MRT", dr[1].ToString()).Replace("CSDL", dr[2].ToString()).Replace("CSN", dr[3].ToString());
                           		smsMsgs.CreateSmsMsg(strMsg, "B", 0, dateNow, new ArrayList {cmoblieno[0]}, this.dolbUser, "", ilen, ipart, 1, out strBatchid);
                                Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + this.dolbUser + ")  上載 " + strBatchid + " 發送成功!  \r\n 短訊內容：\r\n " + strMsg + "\r\n Unicode Msg: " + BitConverter.ToString(Encoding.BigEndianUnicode.GetBytes(strMsg)).Replace("-", "") + "\r\n 發送號碼： " + string.Join(",", (string[])cmoblieno.ToArray(typeof(string))));
...
//                           	{ this.State = 1;
//                           		smsMsgs.CreateSmsMsg(msg.Replace("MRT", dr[0].ToString()).Replace("MRT", dr[1].ToString()).Replace("MRT", dr[2].ToString()), "B", 0, dateNow, new ArrayList {cmoblieno[i]}, this.dolbUser, "", ilen, ipart, 1, out strBatchid);
//                            Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + this.dolbUser + ")  上載 (" + name[2].ToString() + ") " + strBatchid + " 發送成功!  \r\n 短訊內容：\r\n " + msg + "\r\n Unicode Msg: " + BitConverter.ToString(Encoding.BigEndianUnicode.GetBytes(msg)).Replace("-", "") + "\r\n 發送號碼： ");
//                            Files.CicsWriteLog(string.Join(",", (string[])cmoblieno.ToArray(typeof(string))) );
//                            Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + this.dolbUser + ")  上載 (" + name[2].ToString() + ") 完成." + ((strInvildMobile == "") ? "" : " Invalid 號碼: " + strInvildMobile.Trim(",".ToCharArray())) + "\r\n");
//                           	}
//                           }
                    ...
                    if (bResult)
                    {
                        ...
                        {
                            Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + this.dolbUser + ") 上載Agent SMS " + fileName +"成功! （RemoteService）");
                         }
                        else
                        {
                            Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + this.dolbUser + ") 上載Agent SMS " + fileName + "成功! （No RemoteService）");
                        }
                ...
                catch (Exception exp)
                {
                    ...
                    Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + this.dolbUser + ")  停止上載 " + fileName + " !" + exp.Message );
                }
                finally
                {
                    connection.Close();
            ...
            finally
            {
                ...
                try
                {
                    ...
                    SmsActionData actiondata = smsaction.LoadActionByCondition(strcondtion,"AGENT");
