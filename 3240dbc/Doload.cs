            ...
            if (fileName.LastIndexOf("XLS") == fileName.Length - 3 || fileName.LastIndexOf("XLSX") == fileName.Length - 4)
            {
              ...
              //  OleDbConnection connection = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + this.dobtnUploadMode + ";Extended Properties='Excel 8.0;IMEX=1';");
                ...
                catch (Exception exception)
                {
                    ...
                    this.State = 3;
                    flag = false;
                    this.dolbUploadResult = (lang == "en-US") ? "Please upload correct file!" : "請上傳正確格式文件!";// "請上傳正確格式文件!";
                    Files.CicsWriteError(DateTime.Now.ToString("HH:mm:ss") + " Upload: " + fileName + "  Error: " + exception.ToString());
                ...
                if (flag)
                {
                   ...
                   // Percent = 20;
                    ...
                    foreach (DataRow memberRow in rowArray)
                    {
                        ...
                        else
                        {
                            ...
                            if (strMobile.IndexOf("00852") == 0)
                            {
                                ...
                                if (input.Length != 8)
                                {
                                    strErrorMobile = (lang == "en-US") ? "Wrong no. of mobile number digits!" : "電話號碼位數不正確!";// "(電話號碼位數不正確)";
                                }
                                ...
                                {
                                    strErrorMobile = (lang == "en-US") ? "Please input correct mobile number!" : "請輸入正確號碼!";// "(請輸入正確號碼)";
                                }
                            }
                    ...
                    if (flag2)
                    {
                        ...
                        {
                            dolbUploadResult = (lang == "en-US") ? FileName + " upload success,total " + count.ToString() + "!" : FileName + "上傳成功,共上傳 " + count.ToString() + " 個!";// FileName + "上傳成功,共上傳 " + count.ToString() + " 個!";
                        }
                        else if (strErrorMobile.Trim() != "" && strExistMobile.Trim() != "")
                        {
                            dolbUploadResult = (lang == "en-US") ? FileName + " upload success,total " + count.ToString() + ",but：<br>" + strErrorMobile + " error！<br>" + strExistMobile + " existed！" : FileName + "上傳成功,共上傳 " + count.ToString() + " 個,其中號碼：<br>" + strErrorMobile + " 錯誤！<br>" + strExistMobile + " 已存在！";
                        }
                        else if (strExistMobile.Trim() != "")
                        {
                            dolbUploadResult = (lang == "en-US") ? FileName + " upload success,total " + count.ToString() + ",but：<br>" + strExistMobile + " existed！" : FileName + "上傳成功,共上傳 " + count.ToString() + " 個,其中號碼：<br>" + strExistMobile + " 已存在！";
                        }
                        else
                        {
                            dolbUploadResult = (lang == "en-US") ? FileName + " upload success,total " + count.ToString() + ",but：<br>" + strErrorMobile + " error！" : FileName + "上傳成功,共上傳 " + count.ToString() + " 個,其中號碼：<br>" + strErrorMobile + " 錯誤！";
                        }
                        State = 2;
                        Files.CicsWriteLog(DateTime.Now.ToString("HH:mm:ss") + "  (" + dolbUser + ")  " + dolbUploadResult.Replace("<br>", "|"));
                    }
                    else
                    {
                        ...
                        Percent = 0;
                        State = 3;
                        dolbUploadResult = (lang == "en-US") ? " Upload " + FileName + ",Please try a later！" : "上傳" + FileName + ",請稍后再試！";
                    }
                }
            }
            catch (Exception exception2)
