namespace CENTASMS.UserControl
{
    ...
    public class SendTabs : UserControl
    {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                list.Add(new TabItem(Session["LanguageEn"] == "en-US" ? "Send Group SMS" : "群發短訊", string.Concat(new object[] { "SendByGroup.aspx?csindex=", num, "&sIndex=", list.Count })));
                list.Add(new TabItem(Session["LanguageEn"] == "en-US" ? "Instant SMS Records" : "即時發送記錄", string.Concat(new object[] { "SendByGroup.aspx?csindex=", num, "&sIndex=", list.Count })));
                list.Add(new TabItem(Session["LanguageEn"] == "en-US" ? "Scheduled SMS Records" : "預設發送記錄", string.Concat(new object[] { "SendByGroup.aspx?csindex=", num, "&sIndex=", list.Count })));
                list.Add(new TabItem(Session["LanguageEn"] == "en-US" ? "Operator's SMS records" :"落CALL記錄", string.Concat(new object[] { "SendByGroup.aspx?csindex=", num, "&sIndex=", list.Count })));
                if (AppFlag.BulkFunction ==true)
                {
                    list.Add(new TabItem(Session["LanguageEn"] == "en-US" ? "Upload Send" : "上載發送", string.Concat(new object[] { "SendByGroup.aspx?csindex=", num, "&sIndex=", list.Count })));
                }
            ...
            if ((!Users.UserAgentCheck(this.Context.User.Identity.Name.ToUpper())) && (!Users.UserBulkAgentCheck(this.Context.User.Identity.Name.ToUpper())))
            {
                ...
                {
                    list.Add(new TabItem(Session["LanguageEn"] == "en-US" ? "Define Schedule SMS " : "定義預設短訊內容", string.Concat(new object[] { "SendByGroup.aspx?csindex=", num, "&sIndex=", list.Count })));
                }
            }
            //else
            //{
